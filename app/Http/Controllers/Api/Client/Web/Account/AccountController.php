...
class AccountController extends Controller
{
    ...
    public function editNewsInfos(Request $request)
    {
        $user = auth('sanctum')->user();
        ...
        $newsId = Mailinglist::first()->infomaniak_id;
        if ( $request->newsletter == 0 ) {
            $response = Http::withToken(config('services.infomaniak.key'))
                ->withHeaders([
                    'Accept' => 'application/json' ,
                    'Content-Type' => 'application/json' ,
                ])
                ->post('https://newsletter.infomaniak.com/v3/api/1/newsletters/38265/subscribers/filter' , [
                    'filter' => [
                        'status' => 'active' ,
                        'groups' => [$newsId] ,
                        'search' => $user->email ,
            ...
            if ( !$response->successful() ) {
                return response()->json(['status' => 'error' , 'message' => 'An unexcepted error happend.'] , 200);
            }
            ...
            $response = Http::withToken(config('services.infomaniak.key'))
                ...
                ->put("https://newsletter.infomaniak.com/v3/api/1/newsletters/38265/subscribers/$data" , [
                    'status' => 'unsubscribed'
                ]);
            if ( !$response->successful() ) {
                return response()->json(['status' => 'error' , 'message' => 'An unexcepted error happend.'] , 200);
    ...
    public function getNewsinfos(): \Illuminate\Http\JsonResponse
    {
        $user = auth('sanctum')->user();
        if ( !$user ) {
            return response()->json(['status' => 'error' , 'message' => 'You need to be logged.'] , 500);
